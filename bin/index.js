...
function setSelectedFlags(passed_args) {
    ...
    for (const full of fulls) {
        ...
        if (!FLAGS[key])
            badOptionMessage("licon", key);
        //meaning: it needs an argument 
        if (FLAGS[key].args) {
            //if argument is not passed
            if (!value.trim())
                badArgumentMessage("licon", key, false);
...
function transformEntries(entries) {
    ...
    for (const key in OPTIONS) {
        ...
        if (option.selected && option.action)
            option.action(entries);
    }
    // add the ICONS
...
async function getEntries(entry_name) {
    let entries = [];
    try {
...
async function ListEntries(selections) {
    for (const selection of selections) {
        ...
        try {
            ...
            if (err.errno == -20)
                entries.push(new Entry(selection));
            else {
        ...
        if (selections.indexOf(selection) !== selections.length - 1 || OPTIONS.recursive.selected)
            print("");
        if (OPTIONS.recursive.selected) {
